    ...
    <q-card style="width: 700px; max-width: 90vw" class="bg-gray-900 p-4">
      <div class="grid grid-cols-1 md:grid-cols-5 gap-0 md:gap-3">
        <div class="grid grid-cols-2 gap-2 md:grid-cols-1 md:col-span-2">
          <div class="square">
            <AvatarItem :data.sync="avatarProps" :mat.sync="avatarMat"/>
          </div>
          <div>
            ...
            <div class="relative w-full">
              ...
              <div class="w-full" v-for="tab in tabSet">
                <div class="relative w-full flex flex-row flex-wrap" v-if="tab == subTitle">
                  <div class="!w-1/6 square border-gray-800 border-[2px] border-solid" :class="(customColor == color)&&'border-red-700 border-[2px]'" @click="selectColor(color)" v-for="color in avatar[shapeName(tab)].colors" :style="{backgroundColor: color}"></div>
                  <div class="!absolute right-0 bottom-0 !w-1/6 square border-gray-900 border-[2px] border-solid">
                    <q-btn class="w-full h-full p-0 m-0 rounded-full" :class="colorPickerhandle&&'bg-blue-500'" :style="{backgroundColor: customColor}" flat @click="onColorPicker"><q-icon class="w-full h-full" name="tune"/></q-btn>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="relative col-span-3">
          ...
          <div class="w-full flex flex-row flew-wrap mb-2">
            <div class="!w-1/5 bg-white border-[2px] border-solid border-gray-900 square" @click="selectShape(index)" :class="(shapeIndex==index)&&'border-[3px] border-red-700'" v-for="(shape, index) in avatar[shapeName(subTitle)].shapes">
              <ShapeItem :shape.sync="shape" :type.sync="subTitle" :index.sync="index" :mat.sync="avatar[shapeName(subTitle)].mat"/>
            </div>
          </div>
          <div class="w-full my-2 pb-[40px]">
            <q-btn-group class="w-full">
              <q-btn class="w-full p-0 border-gray-400 border-[1px] border-solid" @click="transform(action)" v-for="action in avatar[shapeName(subTitle)].controls">
...
const tabSet=['face','mouth','nose','ears','eyes','iris','brows','glasses','hair','clothes','backs','mustache','beard'];
...
const init = () =>{
  Object.keys(avatarData).map(zone=>{
    if(zone=='hair')
      props = {...props, [zone]:{shapeIndex:1, color:avatarData[zone].colors[0], mat:matProps}};
    else
      props = {...props, [zone]:{shapeIndex:0, color:avatarData[zone].colors[0], mat:matProps}};
  });
  avatarProps.value=props;  
  avatarMat.value = {human: {...avatarmat, scaleupdown:-6}, head: avatarmat};
}
const randomInit = () => {
  tabSet.map(zone=>{
    if(zone=='hair'){
      props = {...props, hair:{shapeIndex: props['face'].shapeIndex, color:avatarData[shapeName(zone)].colors[0], mat: matProps}};
    }
    else{
      let zoneShapeRandIndex = Math.floor(avatarData[shapeName(zone)].shapes.length*Math.random());
      props = {...props, [shapeName(zone)]:{shapeIndex:zoneShapeRandIndex, color:avatarData[shapeName(zone)].colors[0], mat:matProps}};
      if(["ears","eyes","iris","brows"].includes(zone)){
        props = {...props, [`${zone}Left`]:{shapeIndex:zoneShapeRandIndex, color:avatarData[shapeName(zone)].colors[0], mat:matProps}};
      }
    }
  });
  avatarProps.value=props;  
...
const save = () => {
  updateAvatar({props: avatarProps.value, mat:avatarMat.value}, store, router);
