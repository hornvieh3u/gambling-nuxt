<template>
    <div class="contenedor-ruleta">
      <div class="ruleta relative mx-auto" ref="ruleta" :style="{ width: '450px', height: '450px' }">
        <div v-for="i in numeroCasillas" :key="i" class="opcion" :class="'opcion-' + i" :style="{ transform: 'rotate(' + (anguloCasillas * i) + 'deg)', borderBottomWidth: alturaCasilla + 'px', borderRightWidth: tamanyoRuleta / 2 + 'px', borderLeftWidth: tamanyoRuleta / 2 + 'px' }" :data-content="i" :data-ancho="tamanyoRuleta / 2 + 'px'" :data-line="tamanyoRuleta / 2 + 'px'">
          <div class="opcion-numero" :style="{ transform: 'rotate(' + (-anguloCasillas * i) + 'deg)' }"><!-- {{ i }} --></div>
        </div>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    mounted() {
      this.$refs.ruleta.addEventListener('click', () => {
        let numID = 'number-';
        const num = 1 + Math.round(Math.random() * (this.numeroCasillas - 1));
        numID += num;
